...
export function PatientDetails({ patientId, onClose, onDestroyed }) {
  ...
  return (
    <section role="region" aria-label={`Карточка пациента ${patient.name || 'Обезличенный пациент'}`} className="clay-card space-y-5 p-5 sm:p-6">
      ...
      {!destroyed && <div className="flex flex-wrap gap-3">{revealed ? <button type="button" className={SMALL_BUTTON} onClick={clearReveal}><EyeOff aria-hidden="true" size={17} />Скрыть персональные данные</button> : <button type="button" className={SMALL_BUTTON} disabled={busy === 'reveal'} aria-expanded={reasonForm} onClick={() => setReasonForm((current) => !current)}><Eye aria-hidden="true" size={17} />Раскрыть персональные данные</button>}<button ref={destroyTrigger} type="button" className={`${SMALL_BUTTON} border-red-200 text-red-700 hover:border-red-400 hover:text-red-800`} onClick={() => setConfirmDestroy(true)}><ShieldX aria-hidden="true" size={17} />Уничтожить персональные данные</button></div>}
