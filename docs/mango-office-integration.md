...
| `200 {"data":{"outcome":"rejected"}}` в истории запросов | Событие нарушает контракт (неподдерживаемый `location`, битые поля, время вне окна ±) и никогда не будет принято | `SELECT code, entryId, createdAt FROM MangoCallIssue ORDER BY createdAt DESC LIMIT 20` в базе; сверить формат события с официальной документацией |
| Баннер «Телефония молчит» на `/admin/calls` | Более 4 часов рабочего времени без событий: сменились IP API Realtime, callback URL или коннектор отключён | `docker compose logs nginx \| grep '\[mango\]'` покажет отклонённые адреса; сверить allowlist с официальным списком и историю запросов в MANGO |
