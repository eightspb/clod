...
Явное сопоставление врачей и типов приёма поддерживается в `src/lib/medflex-doctors.js`. Для контролируемого обновления discovery используется `bun run medflex:discover`: команда читает токен из локального untracked `.env`, а токен нельзя передавать аргументом командной строки или копировать в вывод, историю shell и отчёты. Результат discovery нужно проверить вручную до изменения allowlist. Команда `bun run medflex:sync-doctors` атомарно связывает этот allowlist с локальными карточками врачей в базе из `ASTRO_DB_REMOTE_URL`. Она создаёт только отсутствующие карточки из проверенных данных сайта, сохраняет ручные изменения, фото и сертификаты существующих карточек и помечает пропавшие связи неактивными вместо удаления.
