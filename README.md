...
До платного запроса Medflex сайт транзакционно создаёт или обновляет зашифрованный профиль `Patient` и локальную `Appointment` со статусом `pending`. Затем результат Medflex проецируется в `confirmed`, `failed` или `needs_review`. Повтор запроса восстанавливает недостающую локальную проекцию по существующему `BookingIntent` и не создаёт вторую платную запись. HTTPS-шаблон Nginx задаёт `proxy_read_timeout 75s` только для `POST /api/appointments/book`; остальные маршруты сохраняют общий timeout.
...
Server-side интеграция принимает подписанные входящие `events/call` и `events/summary`, хранит защищённый журнал и связывает звонящего только с уже существующим пациентом по `CONTACT_FINGERPRINT_KEY`. Неизвестный звонящий не создаёт профиль пациента автоматически. Dashboard и `/admin/calls` показывают активные, отвеченные и пропущенные звонки, долю ответов, ожидание и длительность разговора по московскому времени. Страница звонков отдельно показывает все текущие звонки независимо от фильтра журнала. Номер по умолчанию маскирован; reveal отдельно ограничен и журналируется, а уничтожение номера оставляет только обезличенные показатели.
