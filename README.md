...
Счётчик Яндекс Метрики `26618208` (сайт `odintsovclinic.ru`, зеркало `new.odintsovclinic.ru`) подключён в `Layout.astro` через `src/lib/metrika.js` без баннера согласия: это решение владельца от 10 сентября 2026 года, а собственный трекер по-прежнему ждёт согласия. Вебвизор выключен, `clickmap` и `trackLinks` включены. Цели — JS-события: `phone_click` и `max_click` ставит делегированный обработчик кликов по `tel:` и `max.ru` в лейауте, `booking_open`/`booking_submit`/`booking_confirmed` шлёт `BookingFlow`, `second_opinion_sent` — `SecondOpinionForm`; `reachGoal` молчит, пока `window.ym` не загружен. CSP в `middleware.js` и обоих шаблонах nginx разрешает `https://mc.yandex.ru` в `script-src`/`frame-src` и `https://mc.yandex.ru https://mc.yandex.com` в `connect-src`. Политика конфиденциальности (раздел 9) описывает Метрику и ссылку на opt-out Яндекса. Контракты: `src/lib/metrika.test.js`, `src/components/booking/BookingFlow.metrika.test.jsx`, `src/components/SecondOpinionForm.test.jsx`, `src/middleware.test.js`.

Рекламные кампании ведутся через API Директа v5 от логина `odintsovclinic` (тот же аккаунт владеет счётчиком). OAuth-токен с правами `direct:api` и `metrika:*` хранится только в локальном untracked `.env` (`YANDEX_DIRECT_TOKEN`, `YANDEX_DIRECT_REFRESH_TOKEN`, `YANDEX_OAUTH_CLIENT_ID`, `YANDEX_OAUTH_CLIENT_SECRET`); приложению на сервере он не нужен и в контейнер не передаётся.
...
Карточки дат в расписании имеют ширину 7rem (112 px при базовом размере шрифта): дата и количество мест не переносятся внутри строки. На узких экранах ряд сохраняет горизонтальную прокрутку.
...
Общие CTA без контекста сначала показывают выбор врача. На desktop (от 64rem ширины и 40rem высоты, тот же медиаблок, что и двухколоночная раскладка диалога) шаг «Выберите врача» — сетка 3×3: заголовок и поиск стоят в одной строке, карточка врача — grid с фото и ФИО в первой строке и специальностью под ними на всю ширину, поэтому все девять врачей видны без прокрутки при 1440×800 (контракт `lays out the nine doctors as a three-by-three card grid on desktop` в `e2e/booking-layout.spec.js`). На mobile остаётся вертикальный список с фото слева. Кнопка «Записаться на приём» в секции «Полезные разделы» на `/mammology`, `/gynecology`, `/endocrinology` и `/nutrition` несёт `data-booking-specialty` с id направления из `filters.js`: диалог открывает выбор врача, но список ограничен `matchesFilter` этим направлением, и фильтр сохраняется при «Выбрать другого врача» после пустого расписания (контракт в `src/components/pages/direction-booking-specialty.test.jsx` и `BookingFlow.test.jsx`). CTA на карточке или странице врача передаёт только публичный slug и сразу загружает расписание этого врача; внутренние Medflex ID, токен и секреты не входят в props или HTML. Allowlist охватывает ровно девять профилей: `odintsov`, `prikhodko`, `macuchov`, `skurihin`, `egorova`, `vlasenko`, `zaharova`, `nevzorova` и `kalinina`.

Шапка диалога («Онлайн-запись» и кнопка закрытия) больше не `position: sticky`: `.booking-dialog` всегда `overflow: hidden`, поэтому липкость никогда не работала, а на iOS композитный слой соседнего `.booking-dialog-scroll` закрывал шапку — на iPhone заголовок и крестик не отображались, оставалась пустая белая полоса. Теперь шапка `position: relative` с прежним `z-index: 3`. Контракт — `src/styles/booking-dialog-header.test.js`.
...
### Контракт схемы SQLite: CHECK, внешние ключи, перестроение (Фаза 2 п.12 аудита)

С 9 сентября 2026 года все 30 таблиц проходят строгую проверку в `scripts/init-db.mjs` (`clinicSchemas`): текст `CREATE TABLE`, колонки и индексы должны совпадать с каноническим описанием. Семь таблиц каталога и аналитики (`Doctor`, `Media`, `DoctorCertificate`, `Service`, `AnalyticsSession`, `PageView`, `EventLog`) раньше объявлялись с `REAL` и `INTEGER`-датами (`strftime('%s','now') * 1000`), а drizzle в `src/lib/database-schema.js` писал целые числа и ISO-текст; теперь DDL совпадает с drizzle (`src/test/schema-contract-migration.test.js` сверяет каждую таблицу колонка за колонкой, поэтому дрейф между `database-schema.js` и `init-db` ломает CI). Синхронизация врачей писала в `Media.createdAt` epoch-миллисекунды, а загрузка фото — ISO; теперь только ISO, а старые значения конвертируются при перестроении.

Закрытые множества закреплены `CHECK (col IN (…))`: `Appointment.status/source`, `BookingIntent.status`, `HistoricalVisit.linkStatus/linkMethod/evidenceLevel`, `ImportBatch.mode/status`, `PatientAccess.action`, `MangoCallAccess.action`, `AdminAuthEvent.kind`, `AdminUser.role`, `MangoCall.status`, `MangoCallLeg.state`, `PatientContact.kind`, `PatientNameHistory.reason`, `PatientConsent.type/status`, `MedflexDoctorLink.active`. Внешние ключи объявлены на стабильные связи: `Patient` ← восемь дочерних таблиц и `MangoCall.patientId`, `ImportBatch` ← `ImportIssue`/`HistoricalVisit`/`HistoricalInvoice`, `HistoricalVisit` ← `HistoricalVisitCandidate`/`ImportIssue`, `AnalyticsSession` ← `PageView`/`EventLog`, `MangoCall` ← `MangoCallLeg`/`MangoCallAccess`, `AdminUser` ← `AdminSession`, `Media`/`Doctor` ← `DoctorCertificate`, `Media` ← `Doctor.photoMediaId`. `ImportSourceRow` (115 833 строк, ~400 МБ шифротекста) внешнего ключа не получил: его перестроение удвоило бы файл базы внутри одной транзакции. `@libsql/client` включает `PRAGMA foreign_keys` на каждом соединении, поэтому ключи реально применяются: вставка `PageView` без сессии или `Appointment` со статусом вне списка отклоняется (тесты в `schema-contract-migration.test.js`), а `init-db` после проверки выполняет `PRAGMA foreign_key_check` и печатает предупреждение с таблицами и числом нарушений, не останавливая запуск.

Перестроение выполняется `reconcileSchema`: таблица переименовывается в `<Имя>_schema_migration`, создаётся каноническая, строки копируются с конвертацией (`ISO_DATE`: epoch-миллисекунды и 13-значные строки → ISO-8601; `WHOLE_NUMBER`: `CAST(… AS INTEGER)`), старая удаляется, индексы создаются заново. Перестраивается только таблица, чей текст совпадает с одной из известных прежних форм (`previous`: прежний DDL `init-db` и форма Astro DB с любым литералом `DEFAULT '<ISO>'`) и чьи индексы уже равны каноническим; любая другая форма, лишняя колонка, отсутствующая обязательная колонка, чужой индекс, view или trigger на таблицу останавливают `init-db` с сообщением вида `[init-db] Service table definition drift: expected … but found …` (таблица и обе формы) — так рукописная правка схемы не перезаписывается молча. На время скрипта `PRAGMA foreign_keys = OFF` и `PRAGMA legacy_alter_table = ON`, иначе `ALTER TABLE … RENAME` родительской таблицы переписал бы ссылки в дочерних FK на временное имя. Проверка на копии production-базы (447 МБ, 9 сентября 2026): 27 таблиц перестроены за 1,1 с, `integrity_check` ok, `foreign_key_check` пуст, повторный запуск ничего не меняет.

**Runbook изменения схемы.** 1) Изменить DDL в `init-db.mjs` (`*TableBase` + `withConstraints`) и, если меняются колонки, drizzle-описание в `database-schema.js`, массив `*Columns`/`*Indexes`; прежнюю форму добавить в `previous`. 2) Для новой колонки предпочесть `addColumnIfMissing` (ALTER, без перестроения); индекс на такую колонку создавать после ALTER, как `AdminSession_userId_idx`. 3) Прогнать `bunx vitest run src/test/schema-contract-migration.test.js src/test/appointment-database-migration.test.js src/test/clinic-history-database-migration.test.js`. 4) Перед деплоем снять копию production (`sqlite3 -readonly db.sqlite ".backup /tmp/copy.sqlite"`) и выполнить `ASTRO_DB_REMOTE_URL=file:/tmp/copy.sqlite node scripts/init-db.mjs` дважды: первый запуск печатает перестроенные таблицы, второй — только `Database schema is ready`. 5) Деплой делает бэкап сам (`bun run deploy`); при отказе `init-db` контейнер не стартует, smoke-гейт откатывает образ, база остаётся в прежней форме, потому что перестроение идёт в одной транзакции. `SKIP_DB_INIT=true` пропускает миграцию только для диагностики.

CLI импорта (`bun run clinic:import`) вместо единственного `CLI_FAILED` теперь различает `DISK_FULL` (`SQLITE_FULL`, `ENOSPC`), `OUT_OF_MEMORY` (`SQLITE_NOMEM`), `DATABASE_BUSY` (`SQLITE_BUSY*`, `SQLITE_LOCKED*`) и `DATABASE_CORRUPT` (`SQLITE_CORRUPT`, `SQLITE_NOTADB`) через `src/lib/clinic-import-cli-errors.js`.
...
Состав колонок после переработки: пациенты — «Пациент» (ФИО, под ним `1 карта · 4 визита · 0 проблем` и другие фамилии, если есть), «Телефон», «Обращения» (`первое → последнее` без времени) и действия; записи — «Дата», «Пациент» (ФИО и маска телефона отдельной строкой), «Врач», «Статус» (плашка и источник под ней), «Стоимость» и действия; звонки — «Время», «Звонящий», «Статус», «Линия», «Ожидание», «Разговор» и действия. Склонения считает `plural(count, forms)` из `src/lib/plural.js` (контракт — `src/lib/plural.test.js`): раньше в журнале пациентов выводилось «1 карты · 4 визитов». Легаси-таблицы `/admin/sessions`, `/admin/logs` и `/admin/doctors` на inline-стилях уже плотные и не менялись.
...
│   │   │   ├── HeroSlider.jsx     # Desktop-hero: 3 слайда слева, независимая карусель врачей справа, autoplay 6 с
...
│   │   ├── metrika.js             # Счётчик Яндекс Метрики 26618208, цели-события, без вебвизора
...
Текстовая колонка во всех двухколоночных публичных hero закреплена у верхнего левого края независимо от высоты соседней карточки; на главной внешний grid слайдера использует `items-start`, поэтому текст слайда не центрируется по высоте карусели врачей. Геометрический контракт основных маршрутов и главной проверяет `e2e/hero-alignment.spec.js`.
...
- **Вакансии** (`/vacancies`): открытых вакансий нет, резюме принимаются на `dir@odintsovclinic.ru`; `/job` со старого сайта ведёт сюда
- **Доступная среда** (`/accessibility`): организационные меры (сопровождение администратором, помощь при входе, сопровождающий и переводчик РЖЯ, кнопки размера текста). По подтверждению владельца пандуса на лестнице нет, у входа есть кнопка вызова персонала — так и написано
- **Информация для пациентов** (`/patient-info`): права и обязанности пациента (ст. 19 и 27 323-ФЗ), правила записи и приёма, нормативные документы (323-ФЗ, ПП РФ № 736, 2300-1, 152-ФЗ, программа госгарантий) и контролирующие органы (Росздравнадзор по СПб и ЛО, Комитет по здравоохранению СПб, Роспотребнадзор по СПб) с адресами, телефонами и ссылками на официальные сайты (данные с их сайтов, сентябрь 2026)
...
- **Политика конфиденциальности** расширена до 11 разделов: оператор с ОГРН/ИНН, термины, категории данных, цели и правовые основания (152-ФЗ ст. 6 и 10, 323-ФЗ ст. 13 и 22), локализация в РФ и сроки хранения (аналитика 90 дней, номера звонков 365 дней, резюме 6 месяцев), обработчики (Medflex, MANGO OFFICE, партнёрские лаборатории), права по ст. 14 и срок ответа 10 рабочих дней, cookie и собственный счётчик. Клиника не включена в реестр операторов Роскомнадзора, поэтому такого утверждения в политике нет
- Меню «Пациентам» получило «Акции» и «Информация для пациентов», меню «О клинике» — «Вакансии»; те же ссылки в футере. «Доступная среда» по решению владельца в меню и футер не выносится: на неё ведёт только блок внизу `/patient-info`. Чтобы колонка «Пациентам» футера не растягивалась до девяти пунктов, «Доктора» перенесены в колонку «Направления», а «Блог» и «Контакты» — в колонку «Клиника» (6/7/5 ссылок); эти три ссылки помечены `primary: true` и выделены в футере полужирным тёмным текстом как верхнеуровневые разделы. Новые маршруты добавлены в `e2e/text-overflow.spec.js` и `e2e/hero-viewport-fit.spec.js`
...
Продолжение исправлений — [docs/seo/fixes-2026-09-08.md](docs/seo/fixes-2026-09-08.md). На `/nashi-rezultaty` опубликовано `150+` обученных врачей по уточнению владельца; остальные цифры и распределение по годам сохранены. Счётчики содержат итоговые значения уже в серверном HTML, до гидратации. Видимые ссылки на карточки клиники и `sameAs` используют общие `PRODOCTOROV_CLINIC_URL` и `YANDEX_CLINIC_URL` из `contacts.js`.

Портреты девяти врачей имеют варианты 240/360/480/600 px с `srcset/sizes` в карусели, `DoctorCard` и `HeroDoctorCard`. В карусели `sizes` одинаков для всех слоёв и рассчитан на активный портрет: смена положения не вызывает повторную загрузку другого размера. После замены нормализованных `*-mobile.webp` выполните `bun scripts/resize-doctor-portraits.mjs`: скрипт использует существующий `cwebp` и сохраняет прозрачное полотно 3:4. Порядок обработки: `scripts/normalize-portraits.swift`, затем генерация размеров. При изменении геометрии карточек проверьте `sizes`; подробности — [протокол портретов](docs/seo/portrait-fix-2026-09-08.md).

Основной брендовый `--accent: #1C89A1` сохранён. Для мелкого акцентного текста используется `--accent-ink`, для белого текста кнопок — `--accent-fill`; `text-clay-muted` теперь берёт `--text-muted`. Проверки `accessible-colors.test.js` измеряют контраст скомпилированного CSS, включая hover и focus. Имена ссылок рейтингов и направлений включают видимый текст, группы футера используют H3 под общим H2.
...
В режиме `--live` для `https://odintsovclinic.ru` аудит дополнительно проверяет HTTP `X-Robots-Tag`, robots.txt для Googlebot/YandexBot и ссылку на canonical sitemap. Sitemap и страницы должны отвечать непосредственно HTTP 200. Поле `indexabilityChecked` явно отличает эту проверку от обхода staging и локального HTML; успешный staging-аудит не означает готовность индексации основного домена. `/usd` ведёт на `/prices/full#ultrasound`; пять старых адресов проктологических/урологических услуг без соответствующих страниц возвращают обычный 404 вместо перехода на главную. Это не заявление о прекращении приёма; окончательная карта зависит от актуального состава услуг.
...
- Видимые даты публикации и обновления в блоге и карточках выводятся без сокращения «г.»: `1 апреля 2026`; машинные даты и JSON-LD сохраняют ISO-формат.
...
- Все 30 таблиц под строгой проверкой `init-db`; семь таблиц каталога и аналитики приведены к типам drizzle (ISO-даты, целые), `Media.createdAt` больше не смешивает epoch и ISO
- `CHECK` на 15 enum-колонках и внешние ключи на стабильных связях (кроме `ImportSourceRow`); libsql применяет их на каждом соединении, `init-db` печатает `foreign_key_check`
- `reconcileSchema` перестраивает только известные прежние формы и останавливается с именем таблицы и обеими формами при дрейфе; runbook изменения схемы в разделе «Контракт схемы SQLite»
- CLI импорта различает `DISK_FULL`, `OUT_OF_MEMORY`, `DATABASE_BUSY`, `DATABASE_CORRUPT`
...
- WebP-файлы 1200×675 (около 2,7 МБ на всю серию) используют прежние имена. В поле `image` всех статей добавлена версия `?v=turquoise-20260908`, чтобы посетители получили новую серию, несмотря на 30-дневный кеш `/images/`. Этот же адрес попадает в карточки, обложки статей и их метаданные.
- Изменение редакционное: логика страниц и API не менялась. Проверяются соответствие 40 статей и файлов, декодирование WebP, сборка и отображение блога на desktop/mobile.
...
- **Не изменена**: mobile-карусель `/doctors` — там врач выбирается фильтрами; на desktop `/doctors` карусели нет, только сетка карточек
- **Mobile-карусель главной** («Карусель врачей в начале страницы») получила ту же автопрокрутку 7 сентября 2026 года; контракт — `src/components/pages/Home.test.jsx`
...
- **Прямая связь**: новый блок на главной с гарантией ответа в день обращения; переименован «Врач в мессенджере» → «Лично врачу»; мессенджер клиники — MAX (`MAX_URL`), Telegram не используется
