...
import { PatientPrivateData } from './PatientPrivateData.jsx'
import { PatientVisitDetails } from './PatientVisitDetails.jsx'
import { RevealCountdown } from './RevealCountdown.jsx'
import { isStaffViewer } from '../../lib/admin-role.js'
...
export function PatientDetails({ patientId, onClose, onDestroyed }) {
  ...
  function scheduleHide() {
    if (revealTimer.current) clearTimeout(revealTimer.current)
    revealTimer.current = setTimeout(clearReveal, REVEAL_WINDOW_MS)
    setRevealExpiresAt(Date.now() + REVEAL_WINDOW_MS)
  }
  async function reveal(event) {
    ...
    setActionError('')
    try {
      const result = await requestJson(`/api/admin/patients/${encodeURIComponent(patientId)}/reveal-full`, { method: 'POST', body: JSON.stringify({ scope: 'full', reason: reason.trim() }) })
  ...
  async function resolveIssue(issue) {
    setBusy(`resolve:${issue.id}`)
    try {
      await requestJson(`/api/admin/patient-history/issues/${encodeURIComponent(issue.id)}`, { method: 'PATCH', body: JSON.stringify({ resolved: true }) })
      await load(visits.page.number, issues.page.number, visitsStatus)
    } catch {
      setErrorMessage('Не удалось отметить проблему разрешённой')
    } finally {
      setBusy('')
    }
  }
  async function destroy() {
    ...
    setActionError('')
    try {
      const result = await requestJson(`/api/admin/patients/${encodeURIComponent(patientId)}/personal-data`, { method: 'DELETE', body: JSON.stringify({ confirmation: destroyPhrase.trim(), patientId }) })
      clearReveal()
      setConfirmDestroy(false)
  ...
  return (
    <section role="region" aria-label={`Карточка пациента ${patient.name || 'Обезличенный пациент'}`} className="clay-card space-y-5 p-5 sm:p-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-clay-mint">Защищённая карточка</p><h2 className="mt-2 font-serif text-2xl text-clay-dark">{patient.name || 'Обезличенный пациент'}</h2><p className="mt-1 font-mono text-sm text-clay-admin-muted">{revealed?.profile?.phone || patient.phoneMask || 'Телефон отсутствует'}</p></div><button type="button" className={SMALL_BUTTON} onClick={onClose} aria-label="Закрыть карточку пациента"><X aria-hidden="true" size={17} />Закрыть</button></div>
