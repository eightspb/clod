...
describe('patient records', () => {
  ...
  it('filters patients by data state, activity, visits, and matching issues', async () => {
    ...
    await client.execute({ sql: 'INSERT INTO ImportBatch (id, manifestHash, planHash, mode, status, controlTotals, createdAt, completedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', args: ['batch-filter', 'sha256:manifest', 'sha256:plan', 'apply', 'completed', '{}', FIRST_TIME.toISOString(), FIRST_TIME.toISOString()] })
