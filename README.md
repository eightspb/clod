...
Production-образ содержит CLI и его server-only модули, а сборка выполняет smoke-import команды до публикации образа. Исходные файлы в образ не копируются: для разового production dry-run/apply их монтируют отдельным закрытым каталогом и удаляют после проверки результата.
