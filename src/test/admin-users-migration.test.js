...
describe('named administrator migration', () => {
  it('upgrades a database whose AdminSession predates the userId column', async () => {
    ...
    await client.execute('DROP TABLE AdminSession')
    await client.execute('CREATE TABLE AdminSession (id TEXT PRIMARY KEY, issuedAt TEXT NOT NULL, lastSeenAt TEXT NOT NULL, revokedAt TEXT)')
    await client.execute('CREATE INDEX AdminSession_lastSeenAt_idx ON AdminSession(lastSeenAt)')
