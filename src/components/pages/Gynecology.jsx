...
export function Gynecology({ servicesData = [] }) {
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {GYNECOLOGY_PRICE_CATEGORY.items.map((item) => (
              <div key={item.name} className="clay clay-card flex items-center justify-between gap-4 px-5 py-4">
                ...
                <span className="font-bold text-sm whitespace-nowrap" style={{ color: '#C07050' }}>{formatPriceLabel(item.price, item.isFrom)}</span>
              </div>
            ))}
          </div>
      ...
      <section className="section">
        <div className="container-clay">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-clay-dark mb-3">Заболевания по гинекологии</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {GYNECOLOGY_CONDITIONS.map((c) => (
              <a key={c.href} href={c.href} className="clay clay-card p-6 flex flex-col group hover:shadow-lg transition-shadow">
                ...
                <span className="text-sm font-semibold flex items-center gap-1" style={{ color: '#C07050' }}>
