;(function () {
  ...
  var CONSENT_KEY = 'clod-analytics-consent'
  var CONSENT_EVENT = 'clod:analytics-consent'
  var started = false
  ...
  // The tracker never runs before the visitor grants consent in the banner; a withdrawal
  // stops the timers and drops the visitor identifiers until the next page load.
  if (consent() === 'granted') start()
  document.addEventListener(CONSENT_EVENT, function (e) {
    if (e.detail === 'granted') start()
    if (e.detail === 'denied') stopTracking()
  })
