...
describe('Patients admin view', () => {
  it('keeps patient filters collapsed until an administrator opens them', async () => {
    transport([json(PAGE)])
    render(<Patients />)
    await screen.findByText(PATIENT.name)
    const toggle = screen.getByRole('button', { name: 'Показать фильтры пациентов' })
    expect({ expanded: toggle.getAttribute('aria-expanded'), form: screen.queryByRole('form', { name: 'Фильтры пациентов' }) }).toEqual({ expanded: 'false', form: null })
  })

  it('applies diverse patient filters and summarizes them after collapsing', async () => {
    const calls = transport([json(PAGE), json(PAGE)])
    render(<Patients />)
    await screen.findByText(PATIENT.name)
    fireEvent.click(screen.getByRole('button', { name: 'Показать фильтры пациентов' }))
    fireEvent.change(screen.getByLabelText('Состояние данных'), { target: { value: 'active' } })
    fireEvent.change(screen.getByLabelText('Исторические визиты'), { target: { value: 'with_visits' } })
    fireEvent.change(screen.getByLabelText('Проблемы сопоставления'), { target: { value: 'with_issues' } })
    fireEvent.change(screen.getByLabelText('Последнее обращение с'), { target: { value: '2026-08-01' } })
    fireEvent.change(screen.getByLabelText('Последнее обращение по'), { target: { value: '2026-08-27' } })
    fireEvent.submit(screen.getByRole('form', { name: 'Фильтры пациентов' }))
    await waitFor(() => expect(calls.length).toBe(2))
    expect({ url: String(calls[1][0]), collapsed: screen.queryByRole('form', { name: 'Фильтры пациентов' }), count: screen.getByText('4 активных')?.textContent }).toEqual({ url: '/api/admin/patients?page=1&pageSize=50&piiStatus=active&history=with_visits&issues=with_issues&from=2026-07-31T21%3A00%3A00.000Z&to=2026-08-27T21%3A00%3A00.000Z', collapsed: null, count: '4 активных' })
  })
