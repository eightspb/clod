...
export function PatientDetails({ patientId, onClose, onDestroyed }) {
  ...
  return (
    <section role="region" aria-label={`Карточка пациента ${patient.name || 'Обезличенный пациент'}`} className="clay-card space-y-5 p-5 sm:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-clay-admin-bg p-4"><span className="text-xs text-clay-admin-muted">Внешние карты</span><strong className="mt-1 block text-clay-admin-dark">{countLabel(patient.externalIdentifierCount, 'внешняя карта', 'внешние карты', 'внешних карт')}</strong></div><div className="rounded-2xl bg-clay-admin-bg p-4"><span className="text-xs text-clay-admin-muted">Карты клиники</span><strong className="mt-1 block text-clay-admin-dark">{countLabel(patient.clinicCardCount, 'карта', 'карты', 'карт')}</strong></div><div className="rounded-2xl bg-clay-admin-bg p-4"><span className="text-xs text-clay-admin-muted">Прежние фамилии</span><strong className="mt-1 block text-clay-admin-dark">{countLabel(patient.previousLastNameCount, 'фамилия', 'фамилии', 'фамилий')}</strong></div><div className="rounded-2xl bg-clay-admin-bg p-4"><span className="text-xs text-clay-admin-muted">История</span><strong className="mt-1 block text-clay-admin-dark">{patient.historicalVisitCount} визитов · {patient.issueCount} проблем</strong></div></div>
      ...
      {confirmDestroy && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) closeConfirmation() }}><section role="dialog" aria-modal="true" aria-labelledby="destroy-patient-title" tabIndex={-1} onKeyDown={(event) => handleDialogKeyDown(event, closeConfirmation)} className="clay-card-lg w-full max-w-lg p-6"><ShieldX aria-hidden="true" className="text-red-600" size={28} /><h2 id="destroy-patient-title" className="mt-4 font-serif text-2xl text-clay-dark">Уничтожить персональные данные?</h2><p className="mt-3 text-sm text-clay-muted">Профиль, контакты, паспорт, адрес, прежние фамилии и защищённые детали истории будут удалены безвозвратно. Обезличенная история сохранится.</p><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button autoFocus type="button" className={SMALL_BUTTON} onClick={closeConfirmation}>Отмена</button><button type="button" disabled={busy === 'destroy'} className="inline-flex min-h-11 items-center justify-center rounded-full bg-red-700 px-5 text-sm font-bold text-white hover:bg-red-800 disabled:opacity-50" onClick={destroy}>Уничтожить безвозвратно</button></div></section></div>}
    </section>
  )
}
