...
step_begin 1 6 "проверка свободного места на диске сервера"
...
step_begin 2 6 "git pull (обновление кода с GitHub)"
run_remote "cd ${remote_dir} && git pull"
step_end_ok

step_begin 3 6 "docker system prune (очистка неиспользуемых образов и кэша)"
run_remote "docker system prune -af --filter 'until=24h' 2>/dev/null || true"
step_end_ok

step_begin 4 6 "docker compose up -d --build (сборка и перезапуск — может занять несколько минут)"
run_remote "cd ${remote_dir} && docker compose up -d --build"
step_end_ok

step_begin 5 6 "nginx -s reload (сброс кэша прокси; ждём готовности контейнера)"
reload_nginx_with_retry
step_end_ok

step_begin 6 6 "готово"
