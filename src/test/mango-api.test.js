...
const CLOCK = () => new Date(1_770_000_000_000 + 60_000)
...
describe('MANGO integration API', () => {
  ...
  it('authenticates and normalizes a live event before persistence', async () => {
    ...
    const POST = call.createMangoWebhookEndpoint({ normalize: call.normalizeMangoLiveEvent, records: storage.factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log: vi.fn() })
  ...
  it('uses the summary normalizer and acknowledges non-inbound cleanup', async () => {
    ...
    const POST = final.createMangoSummaryEndpoint({ records: storage.factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log: vi.fn() })
  ...
  it('journals a permanently rejected event by code and acknowledges it with 200', async () => {
    const { call } = await modules()
    const issue = vi.fn(async (input) => input)
    const storage = Object.freeze({ factory: () => Object.freeze({ apply: vi.fn(), issue }) })
    const log = vi.fn()
    const POST = call.createMangoWebhookEndpoint({ normalize: call.normalizeMangoLiveEvent, records: storage.factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log })
    const response = await POST({ request: request(live({ location: 'unsupported-place' })) })
    expect({ status: response.status, body: await body(response), issue: issue.mock.calls[0]?.[0], logged: log.mock.calls.flat() }).toEqual({ status: 200, body: { data: { outcome: 'rejected', reason: 'INVALID_LIVE_EVENT' } }, issue: { code: 'INVALID_LIVE_EVENT', entryId: 'entry-1' }, logged: ['INVALID_LIVE_EVENT'] })
  })

  it('logs the boundary code of a rejected signature without any payload', async () => {
    const { call } = await modules()
    const log = vi.fn()
    const POST = call.createMangoWebhookEndpoint({ normalize: call.normalizeMangoLiveEvent, records: records().factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log })
    await POST({ request: request(live(), { sign: '0'.repeat(64) }) })
    expect(log.mock.calls.flat()).toEqual(['UNAUTHORIZED'])
  })
  ...
  it('rejects an invalid signature before event normalization or storage', async () => {
    ...
    const storage = records()
    const POST = call.createMangoWebhookEndpoint({ normalize, records: storage.factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log: vi.fn() })
  ...
  it('maps malformed domain events, media type, and declared body size safely', async () => {
    const { call } = await modules()
    const make = () => call.createMangoWebhookEndpoint({ normalize: call.normalizeMangoLiveEvent, records: records().factory, credentials: CREDENTIALS, clock: CLOCK, limit: () => ({ allowed: true }), log: vi.fn() })
    ...
    expect([[malformed.status, await body(malformed)], [media.status, await body(media)], [large.status, await body(large)]]).toEqual([[200, { data: { outcome: 'rejected', reason: 'INVALID_LIVE_EVENT' } }], [415, { error: 'UNSUPPORTED_MEDIA_TYPE' }], [413, { error: 'PAYLOAD_TOO_LARGE' }]])
  })
  ...
  it('applies the 300-per-minute limit to the proxy-controlled real IP', async () => {
    ...
    const storage = records()
    const POST = call.createMangoWebhookEndpoint({ normalize: call.normalizeMangoLiveEvent, records: storage.factory, credentials: CREDENTIALS, clock: CLOCK, limit, log: vi.fn() })
    const response = await POST({ request: request(live()) })
