...
Обычные списки и детали показывают имя и маску телефона. Полный номер выдаёт только отдельный административный reveal-запрос: действие журналируется в `PatientAccess`, ограничивается отдельным rate limit и автоматически скрывается в UI через 30 секунд. Уничтожение ПДн необратимо очищает профиль, маску и отпечаток, но оставляет обезличенные UUID, временные метки и историю записей. Этот журнал предназначен для операционного учёта обращений и записей; он не является электронной медицинской картой и не должен хранить диагнозы, результаты исследований или врачебные назначения.
