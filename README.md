...
- **Дубли фактов подряд**: на `/vab` секция «Что важно знать о процедуре» повторяла карточки статистики сразу над ней и удалена, а ряд статистики теперь несёт четыре разных факта (прокол, длительность, восстановление, гистология); на `/second-opinion` шаг «Онколог изучает ваш случай» больше не повторяет «30-40 минут» из карточки над ним. Контракты в `Vab.test.jsx` и `SecondOpinion.test.jsx`
